                ...
                .ForEach((ref Rotation rotation, in AvoidByDistance avoider) =>
                {
                    ...
                    if (collisionWorld.CalculateDistance(pointDistanceInput, out var closestHit )) {
                        ...
                        if (localDistance < avoider.DetectRange) {
                            rotation.Value = Quaternion.Lerp(rotation.Value, Unity.Mathematics.quaternion.RotateZ(radians(90)), 0.02f);
                        } else {
                            rotation.Value = Quaternion.Lerp(rotation.Value, Unity.Mathematics.quaternion.RotateZ(radians(0)), 0.02f);
                        }
                    }
                }).ScheduleParallel();
            Dependency.Complete();
